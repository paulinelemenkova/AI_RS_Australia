...
An %MDPI: For this paper please double check that the following details are provided for all cases: company names and addresses (city, country) of the instruments, agents and softwares used. For USA or Canadian companies, please provide the state name as well (i.e., city name, abbreviated state name, USA/Canada). %Author: yes, correct. I have checked and revised all.
...
{The spatial data variability and differences in their technical characteristics illustrated by the examples above require adjusted approaches, such as ML and AI, to~handle these data in order to obtain geo-information effectively. Therefore, }the RS data {used in this study }were obtained from the %Please ensure meaning has been retained. %Author: yes, retained.
...
(meaning they incorporate geometric correction and topographic correction based on parameters from an OLI TIRS L2SP sensor identifier), %Please ensure meaning has been retained. %Author: yes, retained.
the collection category is T1, the collection number is 2, and ground control points (GCP) are version 5. The spacial %Please ensure meaning has been retained. %Author: yes, retained.
resolution is 30 m for multispectral channels and cirrus (bands 1 to 7 and band 9), 15 m for panchromatic (band 8) and 90 m for the thermal infrared sensor (TIRS) (bands 10 and 11). The~cartographic parameters are the following{: the} data are projected in Universal Transverse Mercator (UTM), the zone is 55 South, the datum and ellipsoid are WGS84. %Please ensure meaning has been retained. %Author: yes, retained.
The~satellite ID is 9 for the scene in 2024 and 8 for the rest of the images, and~the station identifier is LGN for all the data. All images were taken at the nadir in day time. The~Worldwide Reference System (WRS) path is 93 and the row~is 86. %Please ensure meaning has been retained. %Author: yes, retained.
...
\begin{figure}[H]
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{d}) 
  \end{tabular}
  \caption{Results %MDPI: Please add the left bracket in the image, e.g., "1)" should be "(1)".  same as Figures 7-11
%MDPI: We moved the figure after the first citation, please confirm. same as all figures. %Author: yes, I confirm moving after the first citation for this and the following figures. %Author: corrected. The left brackets are added to Figure 6 and all the Figures 7-11. All figures are also revised now. I confirm the moving after first citation.
of image processing of images from 2013 %Please ensure meaning has been retained. %Author: yes, retained.
using four methods: (\textbf{a}) %MDPI: We revised 1234 to abcd, please confirm. %Author: yes, confirm.
random forest (RF); (\textbf{b}) SVM; (\textbf{c})~decision tree classifier; (\textbf{d}) ANN-based MLP classifier. Image processing: author. %MDPI: The figure have incomplete content, please check if it affect reading, please replace it. Same as Figures 7-11 
%Author: yes, corrected. I have checked and revised all figures and their content. Content is now corrected: written clearly the coordinate notations 36:30 S and 38:30 S in the lower and upper left corners of all the images. The content is now correct, the figures include legend and explanations in the text
}
  \label{fig06}
\end{figure}

Since all four ML and {AI} models that were applied belong to the type of supervised classifiers, a~training dataset was necessary for their algorithms. %Please ensure meaning has been retained. %Author: yes, retained.
A training dataset teaches the model to learn the patterns of the different classes that should be discriminated. To~this end, a~sample of unsupervised classification with pixel points was collected via clustering to train the ML and AI algorithms (Figure~\ref{fig07}). %Please ensure meaning has been retained. %Author: yes, retained.
...
\begin{figure}[H]
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{d}) 
  \end{tabular}
   \caption{Image processing for images from 2015 %Please ensure meaning has been retained. %Author: yes, retained.
   	using four methods: \textbf{(a)} RF; \textbf{(b)} SVM; \textbf{(c)} ANN-based MLPC; \textbf{(d)} DTC. Image source: author. %MDPI: The figure have incomplete content, please check if it affect reading, please replace it. %Author: I have checked and revised Figure 7 and its content. Content is corrected: added coordinate notation 38:30 S in the lower left corner of the image. The content is now correct, the figure includes legend and explanations in the text.
}
  \label{fig07}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{d}) 
  \end{tabular}
  \caption{Results of image processing for images from 2017 %Please ensure meaning has been retained. %Author: yes, retained. 
  	using four different methods: (\textbf{a}) random forest; (\textbf{b})~support vector machine (SVM); (\textbf{c}) ANN-based approach using MLP classifier; (\textbf{d}) decision tree classifier. Image processing source: author.} %Author: the figure is revised as in previous cases. I have checked and revised its content. The left bracket is added, as requested. Content is corrected: added coordinate notation 38:30 S in the lower left corner of the image.
  \label{fig08}
\end{figure}
...
{To the east along the Indian Ocean, developed land appears as a segment immediately adjacent to the open coastal spaces. This includes a section of marshes with %Please ensure meaning has been retained. %Author: yes, retained.
	beachfront development that is notable by real estate of compact beach front construction close to the water's border. The~classes of natural vegetation are also observed in other segments of the classified scene and include features such as native forests (closed trees), %Please ensure meaning has been retained. %Author: yes, retained.
	sparse vegetation and scattered trees in drylands. The~regions marked by anthropogenic activities (Class 8) include built-up areas, which comprise constructed commercial and residential buildings, roads and adjacent parking places and related objects of infrastructure. A distribution of farmland and cropland (Class 3) is notable in the eastern areas, where there are more %Please ensure meaning has been retained. %Author: yes, retained.
...
\begin{figure}[H]
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{d}) 
  \end{tabular}
  \caption{Image classification for images from 2024 using four methods: (\textbf{a}) random forest; (\textbf{b}) SVM; (\textbf{c}) ANN-based MLPC; (\textbf{d}) DT classifier. Image processing: author.}
  \label{fig09}
\end{figure}
\vspace{-9pt}
%Author: Figure 9 is revised as in previous cases. I have checked and revised its content. The left bracket is added in all the subplots, as requested. Content is also corrected: added missing coordinate notations 38:30 S and 36:30 S in the lower and upper left corners of the images.

\begin{table}[H]
  \caption{Pixelwise computational statistics on land cover class distribution by spectral bands from~imagery from 2013.   \label{tab03}}%Please ensure meaning has been retained. %Author: yes, retained.
    \vspace{-6pt}
...
\begin{figure}[H]
 ...
 \begin{tabular}[b]{c}
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{d}) 
  \end{tabular}
  \caption{Time series %MDPI: Please add explanation of subfigure abcd. %Author: added year explanations of subfigures a, b, c, and d.
 of the classified images (2013--2024) processed using k-means~clustering. (\textbf{a}) image on 2013; (\textbf{b}) image on 2015; (\textbf{c}) image on 2017; (\textbf{d}) image on 2024. Image processing: author.}
  \label{fig10}
\end{figure}
%Author: Figure 10 is revised as in previous cases. I have checked and revised its content. The left bracket is added in all the subplots, as requested. Content is also corrected: added missing coordinate notations 38:30 S and 36:30 S in the lower and upper left corners of the images  where required.
...
Table~\ref{tab04} shows the evaluated statistical metrics derived from the computations obtained for each multispectral class for images from 2015.%Please ensure meaning has been retained. %Author: yes, retained.

Likewise, the~performances for image classification for years 2017 and 2024 are summarised in Tables~\ref{tab05} and \ref{tab06}. They show the variations in pixels' means and standard deviations for pixels from the Landsat~channel. %Please ensure meaning has been retained. %Author: yes, retained.

The overall trend in land cover type distributions by classes and the dynamics of changes is presented in Table~\ref{tab07}. Here, the~statistical results of the class distribution of the assigned pixels are based on the evaluated signature file for four years and seven multispectral bands, as summarised in Table~\ref{tab07}, which shows %Please ensure meaning has been retained. %Author: yes, retained.
...
{The borders of marshes and wetlands include an edge or the transitional intertidal zone between the main area of Cheetham Wetlands and the transitional zone that includes scrubs, shrubland and bushland as~well as a mixed single tree cover zone. In~contrast to the wetlands, the~areas of farmland and cropland used for agriculture are mostly distributed at higher elevations and are surrounded by the wetlands and marshes in the coastal areas; the farmland and cropland %Please ensure meaning has been retained. %Author: yes, retained.
...
\begin{table}[H]\renewcommand\arraystretch{1.2}
  \caption{Class distributions of the assigned target pixels' %Please ensure meaning has been retained. %Author: yes, I confirm that the meaning has been retained. Bold is removed in the first column, as suggested.
...
{RF is one of the ML models that employs an ensemble method and a decision tree for %Please ensure meaning has been retained. %Author: yes, retained.
...
The accuracy assessment was computed using a chi-square algorithm, which evaluates the probability of the pixel's assignment to the correct class; the results are %Please ensure meaning has been retained. %Author: yes, retained.
...
\begin{figure}[H]
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{d}) 
  \end{tabular}
  \caption{Accuracy analysis %MDPI: Please add explanation of subfigure abcd. %Author: added year explanations of subfigures a, b, c, and d. %Author: Figure 11 is revised as in previous cases. I have checked and revised its content. The left bracket is added in all the subplots, as requested. Content is also corrected: added missing coordinate notations 38:30 S and 36:30 S in the lower and upper left corners of the images where required.
 of image classification (2013--2024) using chi-square algorithm~method. (\textbf{a}) image on 2013; (\textbf{b}) image on 2015; (\textbf{c}) image on 2017; (\textbf{d}) image on 2024. Image processing: author.}
  \label{fig11}
\end{figure}


This test determines at what confidence level each pixel is classified to the correct category of land cover class. Hence, the~reject threshold layer contains the index of the  confidence level computed for each classified cell in the classified Landsat scene. {The results of accuracy assessments evaluate the results observed during image processing. }%Please ensure meaning has been retained. %Author: yes, retained.
...
\funding{The %MDPI: Information regarding the funder and the funding number should be provided. Please check the accuracy of funding data and any other information carefully. %Author: yes, correct. I have checked the information.
publication was funded by the Editorial Office of Journal of Marine Science and Engineering (JMSE), Multidisciplinary Digital Publishing Institute (MDPI) and Institutional Open Access Program (IOAP) participating institution University of Salzburg.}
...
\vspace{-6pt}
\begin{table}[H]
\caption{Initial mean values for pixels in each band assigned to target land cover types for images from 2013 %Please ensure meaning has been retained. %Author: yes, I confirm, retained.
...
\setlength{\tabcolsep}{3.6mm}
  ...
  \multirow{2}{*}{\textbf{Class}} & \multicolumn{7}{c}{\textbf{Initial Mean Values for Multispectral Bands of Landsat}} \\
    ...
    \midrule %Author: bold is deleted in the first column.
    1 & 7944.36 & 8195.91 & 8742.78 & 8861.72 & 12,337.5 & 12,337.4 & 10,304.5 \\
    2 & 8119.86 & 8408.03 & 9026.45 & 9267.03 & 12,909.9 & 13,277 & 11,008.4 \\
    3 & 8295.36 & 8620.15 & 9310.11 & 9672.33 & 13,482.2 & 14,216.6 & 11,712.2 \\
    4 & 8470.86 & 8832.27 & 9593.77 & 10,077.6 & 14,054.6 & 15,156.2 & 12,416.1 \\
    5 & 8646.36 & 9044.38 & 9877.44 & 10,482.9 & 14,627 & 16,095.8 & 13,120 \\
    6 & 8821.86 & 9256.5 & 10,161.1 & 10,888.3 & 15,199.4 &17,035.4 & 13,823.8  \\
    7 & 8997.36 & 9468.62 & 10,444.8 & 11,293.6 & 15,771.7 & 17,975 & 14,527.7 \\
    8 & 9172.86 & 9680.74 & 10,728.4 & 11,698.9 & 16,344.1 & 18,914.6 & 15,231.5 \\
    9 & 9348.36 & 9892.86 & 11,012.1 & 12,104.2 & 16,916.5 & 19,854.2 & 15,935.4 \\
    10 & 9523.86 & 10,105 & 11,295.8 & 12,509.5 & 17,488.8 & 20,793.8 & 16,639.3 \\
\bottomrule
\end{tabular}
\end{adjustwidth}
...
\begin{table}[H]
  \caption{Initial mean values for pixels in each band assigned to target land cover types for images from 2015 %Please ensure meaning has been retained. %Author: yes, I confirm, retained.
...
%\centering %% If there is a figure in wide page, please release command \centering
 \setlength{\tabcolsep}{3.6mm}
  ...
  \multirow{2}{*}{\textbf{Class}} & \multicolumn{7}{c}{\textbf{Initial Mean Values for Multispectral Bands of Landsat}} \\
     & \textbf{1---Coastal Aerosol} & \textbf{2---Blue} & \textbf{3---Green} & \textbf{4---Red} & \textbf{5---NIR} & \textbf{6---SWIR-1} & \textbf{7---SWIR-2} \\
    \midrule %Author: bold is deleted in the first column.
    1 & 7964.53 & 8167.16 & 8635.84 & 8723.91 & 12,003.5 & 11,922.3 & 10,077.8 \\
    2 & 8143.64 & 8371.91 & 8901.87 & 9095.84 & 12,597.6 & 12,891.5 & 10,824.4  \\
    3 & 8322.74 & 8576.66 & 9167.9 & 9467.76 & 13,191.7 & 13,860.6 & 11,571 \\
    4 & 8501.84 & 8781.41 & 9433.92 & 9839.69 & 13,785.8 & 14,829.8 & 12,317.6 \\
    5 & 8680.94 & 8986.16 & 9699.95 & 10,211.6 & 14,379.8 & 15,798.9 & 13,064.2 \\
    6 & 8860.04 & 9190.91 & 9965.98 & 10,583.5 & 14,973.9 & 16,768.1 & 13,810.8 \\
    7 & 9039.15 & 9395.66 & 10,232 & 10,955.5 & 15,568 & 17,737.2 & 14,557.5 \\
    8 & 9218.25 & 9600.41 & 10,498 & 11,327.4 & 16,162.1 & 18,706.4 & 15,304.1 \\
    9 & 9397.35 & 9805.16 & 10,764.1 & 11,699.3 & 16,756.2 & 19,675.5 & 16,050.7 \\
    10 & 9576.45 & 10,009.9 & 11,030.1 & 12,071.2 &17,350.2 & 20,644.7 & 16,797.3 \\
\bottomrule
\end{tabular}
\end{adjustwidth}
...
\setlength{\tabcolsep}{3.6mm}
  ...
  \multirow{2}{*}{\textbf{Class}} & \multicolumn{7}{c}{\textbf{Initial Mean Values for Multispectral Bands of Landsat}} \\
     & \textbf{1---Coastal Aerosol} & \textbf{2---Blue} & \textbf{3---Green} & \textbf{4---Red} & \textbf{5---NIR} & \textbf{6---SWIR-1} & \textbf{7---SWIR-2} \\
    \midrule %Author: bold is deleted in the first column.
    1 & 7956.33 & 8207.65 & 8763.65 & 8860.93 & 12,306 & 11,946.7 & 10,001.2 \\
    2 & 8117.93 & 8401.46 & 9023.47 & 9249.65 & 12,895.4 & 12,888 & 10,677.9 \\
    3 & 8279.53 & 8595.27 & 9283.28 & 9638.37 & 13,484.7 & 13,829.4 & 11,354.6 \\
    4 & 8441.13 & 8789.08 & 9543.09 & 10,027.1 & 14,074.1 & 14,770.7 & 12,031.3 \\
    5 & 8602.73 & 8982.89 & 9802.91 & 10,415.8 & 14,663.4 & 15,712 & 12,708 \\
    6 & 8764.32 & 9176.7 & 10,062.7 & 10,804.5 & 15,252.8 & 16,653.4 & 13,384.7 \\
    7 & 8925.92 & 9370.51 & 10,322.5 & 11,193.3 & 15,842.1 & 17,594.7 & 14,061.4 \\
    8 & 9087.52 & 9564.32 & 10,582.3 & 11,582 & 16,431.5 & 18,536.1 & 14,738.1 \\
    9 & 9249.12 & 9758.14 & 10,842.2 & 11,970.7 & 17,020.8 & 19,477.4 & 15,414.8 \\
    10 & 9410.72 & 9951.95 & 11,102 & 12,359.4 & 17,610.2 & 20,418.7 & 16,091.6 \\
\bottomrule
\end{tabular}
\end{adjustwidth}
...
\begin{table}[H]
  \caption{Initial mean values for pixels in each band assigned to target land cover types for images from 2024 %Please ensure meaning has been retained. %Author: yes, retained.
...
\setlength{\tabcolsep}{3.6mm}
  ...
  \multirow{2}{*}{\textbf{Class}} & \multicolumn{7}{c}{\textbf{Initial Mean Values for Multispectral Bands of Landsat}} \\
     & \textbf{1---Coastal Aerosol} & \textbf{2---Blue} & \textbf{3---Green} & \textbf{4---Red} & \textbf{5---NIR} & \textbf{6---SWIR-1} & \textbf{7---SWIR-2} \\
    \midrule %Author: bold is deleted in the first column.
     1 & 7883.17 & 8161.79 & 8731.22 & 8868.39 & 12,420.6 & 11,749.3 & 9919.64 \\
     2 & 8066.83 & 8374.7 & 9010.16 & 9289.44 & 13,074.4 & 12,751.4 & 10,645.2 \\
     3 & 8250.49 & 8587.61 & 9289.1 & 9710.5 & 13,728.3 & 13,753.5 & 11,370.7 \\
     4 & 8434.15 & 8800.53 & 9568.04 & 10,131.5 & 14,382.2 & 14,755.5 & 12,096.3 \\
     5 & 8617.81 & 9013.44 & 9846.97 & 10,552.6 & 15,036 & 15,757.6 & 12,821.8 \\
     6 & 8801.47 & 9226.35 & 10,125.9 & 10,973.7 & 15,689.9 & 16,759.6 & 13,547.4 \\
     7 & 8985.13 & 9439.26 & 10,404.9 & 11,394.7 & 16,343.7 & 17,761.7 & 14,272.9 \\
     8 & 9168.79 & 9652.17 & 10,683.8 & 11,815.8 & 16,997.6 & 18,763.8 & 14,998.4 \\
     9 & 9352.45 & 9865.08 & 10,962.7 & 12,236.8 & 17,651.4 & 19,765.8 & 15,724 \\
    10 & 9536.11 & 10,078 & 11,241.7 & 12,657.9 & 18,305.3 & 20,767.9 & 16,449.5 \\
\bottomrule
\end{tabular}
...
\newblock Available online: \url{https://www.abs.gov.au/statistics/people/population/national-state-and-territory-population/latest-release} (accessed on 20 June 2024). %MDPI: Please add the day. %Author: information is added (20 June 2024)
...
\newblock {Victoria’s Regional Catchment Strategies}.
\newblock Available online: \url{https://portphillipwesternport.rcs.vic.gov.au/} %MDPI: Please provide the link and accessed date. %Author: information is added.
