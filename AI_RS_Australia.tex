...
The RS data were obtained from the sources of Landsat 8-9 Landsat Operational Land Imager and Thermal Infrared Sensor (OLI-TIRS) United States Geological Survey (USGS). The main metadata of the satellite images are summarised in Table \ref{tab01}. 
...
\begin{figure}[H]
    ...
    \end{center}
    \caption{Workflow methodology. Software: RStudio version 2024.04.2-764, R 3.6.0. Diagram source: author.}
    \label{fig05}
\end{figure}
...
Table \ref{tab04} shows the evaluated statistical metrics derived from the computations obtained for each multispectral class for 2015. Likewise, the performance of the image classification for years 2017 and 2024 were summarised in Table \ref{tab05} and Table \ref{tab06}. It shows the variations in pixels's means and standard deviation for pixels by Landsat channel. The overall trend in land cover type distribution by classes and the dynamics of changes is presented in Table \ref{tab07}. Here, the statistical results of the class distribution of the assigned pixels are based on the of the evaluated signature file by four years and seven multispectral bands as summarised in Table \ref{tab07} showing target land cover types for 2013--2024. Land cover class 1 (water -- Port Phillip Bay, lakes, rivers) demonstrated fluctuations with a slight increase in 2015 and 2024 which points at climate effects and higher precipitation. 
...
The accuracy assessment computed using chi-square algorithms which evaluates the probability of the pixel's assignment to the correct class is shown in Figure \ref{fig11}. Accuracy assessment was performed using the computed reject raster map which presents the threshold. The threshold evaluates the limits according to which the pixels are assigned to the target classes correctly. It is possible using a chi square test calculated on each discriminant result at tested threshold levels of confidence. This test determines at what confidence level each pixels are classified to the correct categories of the land cover classes. Hence, the reject threshold layer contains the index to the  confidence level computed for each classified cell in the classified Landsat scene. Additionally, the confusion separability matrices were computed to evaluate the correctness of the pixel's classification for all the years: 2013 and 2015 (Figure \ref{fig12}) and 2017 and 2024 (Figure \ref{fig13}).
...
\section[\appendixname~\thesection]{Class separability matrices}\label{AppB}
