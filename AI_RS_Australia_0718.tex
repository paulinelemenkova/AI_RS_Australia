...
Table \ref{tab04} shows the evaluated statistical metrics derived from the computations obtained for each multispectral class for 2015. Likewise, the performance of the image classification for years 2017 and 2024 were summarised in Table \ref{tab05} and Table \ref{tab06}. It shows the variations in pixels's means and standard deviation for pixels by Landsat channel. 
...
In general, the SVM and RF models’ performance demonstrated similar results which is caused due to the technical parameters of these algorithms. The most distinct and accurate (over 98\% of accuracy) performance was noted by the AI algorithm of MLPC. 
...
The accuracy assessment computed using chi-square algorithms which evaluates the probability of the pixel's assignment to the correct class is shown in Figure \ref{fig11}. Accuracy assessment was performed using the computed reject raster map which presents the threshold. 
